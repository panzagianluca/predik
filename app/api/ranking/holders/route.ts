...
export async function GET(request: NextRequest) {
  try {
    ...
    logger.log(`💎 Fetching holders ranking from Myriad V2 API`);
    ...
    logger.log(`📊 Fetching holders from ${markets.length} markets`);
    ...
    const holdersPromises = markets.slice(0, 20).map(async (market: any) => {
      try {
        ...
        logger.error(`Error fetching holders for market ${market.id}:`, err);
        return null;
      }
    });
    ...
    logger.log(`💎 Top ${rankedHolders.length} holders calculated`);
    ...
    logger.error("❌ Error fetching holders ranking:", error);
    return NextResponse.json(
      {
        error: "Failed to fetch holders ranking",

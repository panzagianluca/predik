...
```typescript
// drizzle/schema.ts
export const marketHolders = pgTable(
  "market_holders",
  {
  ...
  (table) => ({
    uniqueHolder: uniqueIndex("unique_market_outcome_holder").on(
      table.marketId,
      table.outcomeId,
      table.holderAddress,
    ),
...
export async function indexMarketHolders(marketId: number) {
  ...
  for (const holderAddress of topHolders) {
    try {
      ...
      const userShares = await pm
        .getContract()
        .methods.getUserMarketShares(marketId, holderAddress)
        .call();
      ...
      for (
        ...
        const sharesFormatted =
          Number(sharesRaw) / Math.pow(10, market.token.decimals || 6);
        ...
        if (sharesFormatted > 0) {
          ...
          await db
            ...
            .onConflictDoUpdate({
              target: [
                marketHolders.marketId,
                marketHolders.outcomeId,
                marketHolders.holderAddress,
              ],
              set: {
                shares: sharesFormatted.toString(),
                usdValue: usdValue.toFixed(2),
...
export async function GET(
  ...
  try {
    ...
    for (let outcomeIndex = 0; outcomeIndex < outcomes.length; outcomeIndex++) {
      ...
      for (const holderAddress of topHolders) {
        try {
          const { outcomeShares } = await getHolderShares(
            marketId,
            holderAddress,
          );
      ...
      console.log(
        `  ✅ Outcome ${outcomeIndex}: ${top20.length} holders with shares`,
      );
    }

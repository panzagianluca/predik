...
/**
 * @title PredikUSD - Mock USD Token for Predik
 * @notice PredikUSD - Platform currency for Predik prediction markets on BNB Mainnet
 * @dev ERC-20 token with 6 decimals (matching USDT) and a faucet function for demo purposes
 */
contract PredikUSD is ERC20, Ownable {
    /// @notice Number of decimals (matching real USDT)
    uint8 private constant DECIMALS = 6;

    /// @notice Amount minted per faucet call (1,000 PredikUSD)
    uint256 public constant FAUCET_AMOUNT = 1_000 * 10 ** DECIMALS;

    /// @notice Initial mint amount for deployer (1,000,000 PredikUSD)
    uint256 private constant INITIAL_MINT = 1_000_000 * 10 ** DECIMALS;

    /// @notice Cooldown period between faucet calls (24 hours)
     ...
     */
    constructor() ERC20("PredikUSD", "pUSD") Ownable(msg.sender) {
    ...
    /**
     * @notice Faucet function - Mints 1,000 PredikUSD to caller
     * @dev Has 24-hour cooldown to prevent abuse
     */
    function faucet() external {
        uint256 lastClaim = lastFaucetClaim[msg.sender];
        if (lastClaim != 0) {
            require(block.timestamp >= lastClaim + FAUCET_COOLDOWN, "PredikUSD: Faucet cooldown active");
        }

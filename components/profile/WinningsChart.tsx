...
export function WinningsChart({
  ...
  useEffect(() => {
    if (!mounted || !chartContainerRef.current || transactions.length === 0)
      return;
    ...
    const dataPoints: WinningsDataPoint[] = sortedTransactions.map((tx) => {
      ...
      const investedNumber = Number(
        formatUnits(cumulativeInvested, tokenDecimals),
      );
      const withdrawnNumber = Number(
        formatUnits(cumulativeWithdrawn, tokenDecimals),
      );
        ...
        tooltip.innerHTML = `
          ...
          <div class="space-y-1">
            <div class="flex items-center justify-between gap-4">
              ...
              <span class="font-bold" style="color: ${plColor}">${plSign}${plValue.toFixed(
                2,
              )} ${tokenSymbol}</span>
            </div>
            <div class="flex items-center justify-between gap-4">
              <span class="opacity-80">Invested:</span>
              <span class="font-semibold">${dataPoint.cumulativeInvested.toFixed(
                2,
              )} ${tokenSymbol}</span>
            </div>
            <div class="flex items-center justify-between gap-4">
              <span class="opacity-80">Withdrawn:</span>
              <span class="font-semibold">${dataPoint.cumulativeWithdrawn.toFixed(
                2,
              )} ${tokenSymbol}</span>
            </div>
          </div>
  ...
  if (!mounted) {
    return (
      <div
        className={`w-full h-[300px] flex items-center justify-center bg-muted/10 rounded-lg ${className}`}
      >
        <LogoSpinner size={40} />
      </div>
    );
  ...
  if (transactions.length === 0) {
    return (
      <div
        className={`w-full h-[300px] flex flex-col items-center justify-center bg-muted/10 rounded-lg ${className}`}
      >

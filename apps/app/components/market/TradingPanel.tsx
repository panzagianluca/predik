...
export function TradingPanel({
  ...
  const loadUserPosition = async () => {
    try {
      ...
      if (shares > 0) {
        ...
        const percentChange =
          ((currentValue - neutralValue) / neutralValue) * 100;
  ...
  const calculateTrade = async () => {
    ...
    try {
      ...
      const maxProfit = tradeType === "buy" ? shares - tradeAmount : 0; // For sells, we can't calculate max profit without knowing original purchase price

      const maxProfitPercent =
        tradeType === "buy" ? (maxProfit / tradeAmount) * 100 : 0;
  ...
  const handleExecuteTrade = async () => {
    ...
    if (tradeType === "buy" && tradeAmount > balance) {
      ...
      setError(
        `Saldo insuficiente de ${market.token.symbol}. Tienes ${balance.toFixed(
          2,
        )}, necesitas ${tradeAmount}`,
      );
      return;
    }
      ...
      <Card className={className}>
        <CardContent className="flex items-center justify-center h-64">
          <p className="text-muted-foreground">
            No hay resultados disponibles para este mercado
          </p>
        </CardContent>
      </Card>
    );
            ...
            <div className="grid grid-cols-2 gap-2 relative">
              {market.outcomes.map((outcome) => (
                <Button
                  key={outcome.id}
                  variant={
                    selectedOutcome?.id === outcome.id ? "default" : "outline"
                  }
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center justify-between text-sm">
                  ...
                  <div className="flex items-center gap-2">
                    <span className="font-medium">
                      {(calculation.avgPrice * 100).toFixed(2)}%
                    </span>
                    <span
                      className={cn(
                        "text-xs",
                ...
                <div className="flex items-center justify-between text-sm">
                  <span className="text-muted-foreground">
                    Acciones {tradeType === "buy" ? "Recibidas" : "Vendidas"}
                  </span>
                  <span className="font-medium">
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  <span
                    className={cn(
                      "font-semibold",
                      userPosition.dollarChange >= 0
                        ? "text-green-600"
                        : "text-red-600",
                    )}
                  >
                    {userPosition.dollarChange >= 0 ? "+" : ""}$
                    {userPosition.dollarChange.toFixed(2)}
                  </span>
                  <span
                    className={cn(

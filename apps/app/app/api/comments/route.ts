...
import { comments, users, commentVotes, notifications } from "@predik/database";
...
export async function GET(request: NextRequest) {
  try {
    ...
    const uniqueAddresses = [
      ...new Set(allUserAddresses.map((addr) => addr.toLowerCase())),
    ];
    ...
    const formatComment = (comment: (typeof topLevelComments)[0]): Comment => {
      ...
      const avatarUrl = profile?.customAvatar
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (parentId) {
      ...
      if (
        parentComment &&
        parentComment.userAddress.toLowerCase() !== userAddress.toLowerCase()
      ) {
